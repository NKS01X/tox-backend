...
def train_models(X_selected_df: pd.DataFrame, y: pd.DataFrame, labels: list[str]):
    ...
    for label in labels:
        log.info(f"Training models for: {label}")
        mask      = y[label].notna()
        X_label   = X_selected_df[mask]
        y_label   = y.loc[mask, label]

        # Skip if a label is fully empty or has only one class
        if len(y_label.unique()) < 2:
            log.warning(f"  Skipping {label} due to single class/empty data.")
            continue
        ...
        log.info(f"  LightGBM AUC: {auc:.4f}")
        ...
        log.info(f"  XGBoost   AUC: {auc:.4f}")
        xgb_models[label] = xgb

    return lgb_models, xgb_models

def save_pipeline(path: str, lgb_models, xgb_models, scaler, selector, selected_columns, labels):
    pipeline = {
        "models":           lgb_models,
        ...
        "labels":          list(lgb_models.keys()), # Only save labels we successfully trained
    }
...
def main():
    ...
    parser.add_argument("--out", default=str(Path(__file__).parent / "toxicity_model.pkl"), help="Output pickle path")
    args = parser.parse_args()

    # 1. Load Data
    df = load_dataset(args.csv)

    # 2. Extract Features FIRST (This aligns data if SMILES fail)
    X, df = build_features(df)

    # 3. Extract Labels from the cleaned DataFrame
    ...
    # 6. Save Pipeline
    save_pipeline(args.out, lgb_models, xgb_models, scaler, selector, selected_columns, labels)

# FIX: Ensured main execution block is at the bottom of the script
